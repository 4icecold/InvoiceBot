...
class Game:
    ...
    async def check_message(self, message):
        ...
        if self.frenzy_end_time and now < self.frenzy_end_time:
            current_chance *= self.frenzy_multiplier

        if random.uniform(0, 100) <= current_chance:
            self.active = False
            await self.channel.send(f"{message.author.mention}, you have won the message game! Make a ticket in <#1217373542136418374> to claim your prize.")
    ...
    async def start_frenzy(self, length, multiplier, interaction):
        ...
        self.frenzy_end_time = datetime.now() + timedelta(seconds=length)
...
@bot.tree.command(name="unban_user_from_message_game", description="Unbans a user from playing the message game")
...
async def unban_user_from_message_game(interaction: discord.Interaction, game_id: int, user: discord.User):
    ...
    if game_id in games:
        ...
        games[game_id].banned_users.remove(user.id)
        log_channel = bot.get_channel(log_channel_id)
        if log_channel:
            embed = discord.Embed(
...
async def active_games(interaction: discord.Interaction):
    ...
    if active_game_ids:
        await interaction.response.send_message(f"Active games: {', '.join(map(str, active_game_ids))}", ephemeral=True)
    else:
...
async def start_frenzy(interaction: discord.Interaction, game_id: int, length: int, multiplier: float, frenzy_chance: float):
    ...
    if game.frenzy_end_time and datetime.now() < game.frenzy_end_time:
        await interaction.response.send_message("A frenzy is already active for this game.", ephemeral=True)
        return
